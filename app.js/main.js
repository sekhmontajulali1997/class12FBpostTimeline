...
const allPosts = getlsdata('posts');
let content= '';
...
if (!objectconvertb.Athourname || !objectconvertb.Athourimage) {
  ...
  const getdataformls = getlsdata('posts');

getdataformls.push(

  {
    AthourName: objectconvertb.Athourname,
    AthourImage: objectconvertb.Athourimage,
    Postcontent: objectconvertb.postcontent ?? null,
    Postimage: objectconvertb.Postimage ?? null,
    Posttime: Date.now(),
  }

  
)
setlsdata('posts', getdataformls );
 showposts();
e.target.reset();
